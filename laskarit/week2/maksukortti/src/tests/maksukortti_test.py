...
class TestMaksukortti(unittest.TestCase):
    ...
    def test_neg_summan_laataminen_ei_muuta_kortin_saldoa(self):
        self.kortti.lataa_rahaa(-100)

        self.assertEqual(str(self.kortti), "Kortilla on rahaa 10.00 euroa")

    def test_pystyy_ostamaan_edullisen_lounaan_kun_rahaa_vain_edullisen_lounaan_verran(self):
        kortti = Maksukortti(250)
        kortti.syo_edullisesti()

        self.assertEqual(str(kortti), "Kortilla on rahaa 0.00 euroa")

    def test_pystyy_ostamaan_maukkaan_lounaan_kun_rahaa_vain_maukkaan_lounaan_verran(self):
